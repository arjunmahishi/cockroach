...
func makeGenerateCmd(runE func(cmd *cobra.Command, args []string) error) *cobra.Command {
	generateCmd := &cobra.Command{
        ...
        dev generate dashboards    # Generate TypeScript dashboard files from YAML configurations
`,
...
func (d *dev) generateDashboards(cmd *cobra.Command) error {
	...
	if err := d.exec.CommandContextInheritingStdStreams(ctx, "bazel", "build", "//pkg/cmd/dashgen"); err != nil {
	...
	dashgenPath := filepath.Join(bazelBin, "pkg/cmd/dashgen/dashgen_/dashgen")
	
	d.log.Printf("Generating dashboards to %s", outputDir)
	if err := d.exec.CommandContextInheritingStdStreams(ctx, dashgenPath, outputDir); err != nil {
